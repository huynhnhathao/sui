    ...
    pub fn new(approved_actions: Vec<BridgeAction>) -> BridgeResult<Self> {
        ...
        for action in approved_actions {
            if !action.is_governace_action() {
                return Err(BridgeError::ApprovedActionIsNotGovernanceAction(action));
            }
...
mod tests {
    ...
    async fn test_governance_verifier() {
        ...
        assert!(matches!(
            GovernanceVerifier::new(vec![action_1, action_2, action_4]).unwrap_err(),
            BridgeError::ApprovedActionIsNotGovernanceAction(..)
        ));
    }
}
